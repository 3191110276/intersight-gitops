"""
Base Policy class for all Intersight policy objects.

This module provides the base Policy class that all specific policy 
implementations inherit from. It handles common policy attributes and
provides a foundation for policy-specific implementations.
"""
...
class BasePolicy(ObjectType):
    """
    Base class for all Intersight policy objects.
    
    This class provides common functionality for all policy types including:
    - Organization field handling
    - Common policy attributes (Name, Description, Tags)
    - Profile relationship management
    - Policy-specific validation and transformation
    """
    
    def __init__(self, api_client=None, openapi_schema=None, validation_level=ValidationLevel.STRICT):
        """Initialize the base policy object."""
        super().__init__(api_client, openapi_schema, validation_level)
        
        # Set up transformations for Organization field
        self.add_import_transform('Organization', self._transform_organization_for_import)
        self.add_export_transform('Organization', self._transform_organization_for_export)
    
    @property
    def folder_path(self) -> str:
        """Return the base folder path for policies."""
        return "policies"
    
    def _get_common_policy_fields(self) -> Dict[str, FieldDefinition]:
        """
        Get common fields that all policies share.
        
        Returns:
            Dictionary of common policy field definitions
        """
        return {
    ...
    def _transform_organization_for_export(self, value):
        """
        Transform organization reference for export.
        
        Converts organization object reference to simple string name for YAML export.
        
        Args:
            value: Organization reference (dict object or string name)
            
        Returns:
            Simple organization name string
        """
        if isinstance(value, dict):
            # Extract name from organization object reference
            name = value.get('Name')
            if name:
                return name
            
            # Fallback: if no Name, try to resolve MOID to name
            moid = value.get('Moid')
            if moid and self.api_client and hasattr(self.api_client, 'organization_resolver'):
                try:
                    resolved_name = self.api_client.organization_resolver.resolve_moid_to_name(moid)
                    if resolved_name:
                        return resolved_name
                except Exception as e:
                    logger.warning(f"Could not resolve organization MOID {moid} to name: {e}")
            
            # Final fallback: use MOID if no name available
            return moid or 'unknown'
        elif isinstance(value, str):
            # Already a string name, return as-is
            return value
        else:
            logger.warning(f"Unknown organization reference format for export: {value}")
            return str(value) if value else 'unknown'
    
    def generate_filename(self, obj: Dict[str, Any], organization_name: str = None) -> str:
        """
        Generate a filename for a policy object.
        
        Policies are named with organization prefix: {org_name}_{policy_name}.yaml
        
        Args:
            obj: The policy object dictionary
    ...
    def export(self, output_dir: str) -> Dict[str, Any]:
        """
        Export policies from Intersight to YAML files.
        ...
        try:
            ...
            # Query all policies from Intersight
            raw_objects = self.api_client.query_objects(self.object_type)
            logger.info(f"Retrieved {len(raw_objects)} raw {self.display_name} policies from Intersight")
            
            # Filter out system default policies
            filtered_objects = self.filter_system_defaults(raw_objects)
            logger.info(f"Filtered to {len(filtered_objects)} user-defined {self.display_name} policies")
            
            # Only create directory when we have objects to export
            if not filtered_objects:
                logger.info(f"No user-defined {self.display_name} policies to export, skipping folder creation")
                return results
            
            # Process each policy
            for policy_obj in filtered_objects:
                try:
                    # Extract user-defined values
                    user_data = self._extract_user_defined_values(policy_obj)
                    ...
                    policy_dir = os.path.join(output_dir, self.folder_path)
                    ...
                    logger.debug(f"Exported {self.display_name} policy '{processed_data.get('Name')}' to {file_path}")
                    
                except Exception as e:
                    error_msg = f"Failed to export {self.display_name} policy '{policy_obj.get('Name', 'unknown')}': {e}"
            ...
            error_msg = f"Failed to export {self.display_name} policies: {e}"
            logger.error(error_msg)
            results['errors'].append(error_msg)
            results['error_count'] += 1
    ...
    def import_objects(self, yaml_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Import policies from YAML data to Intersight.
        ...
        Args:
            yaml_data: List of policy dictionaries from YAML files
        ...
        try:
            ...
            # Get current policies from Intersight
            ...
            # Create lookup maps (API might use lowercase 'name' like organizations)
            ...
            logger.debug(f"Current {self.display_name} policies from API: {list(current_by_name.keys())}")
            logger.debug(f"YAML {self.display_name} policies: {list(yaml_by_name.keys())}")
            ...
            error_msg = f"Failed to import {self.display_name} policies: {e}"
            logger.error(error_msg)
            results['errors'].append(error_msg)
            results['error_count'] += 1
        ...
        for name in to_create:
            try:
                ...
                if validation_errors:
                    error_msg = f"Validation failed for {self.display_name} policy '{name}': {validation_errors}"
                    logger.error(error_msg)
                    results['errors'].append(error_msg)
                    results['error_count'] += 1
                    continue
                ...
                logger.info(f"Created {self.display_name} policy: {name}")
                
            except Exception as e:
                error_msg = f"Failed to create {self.display_name} policy '{name}': {e}"
                logger.error(error_msg)
                results['errors'].append(error_msg)
                results['error_count'] += 1
        ...
        for name in to_update:
            try:
                ...
                if self._objects_are_equivalent(current_obj, yaml_obj):
                    logger.debug(f"{self.display_name} policy '{name}' is already up to date")
                    continue
                ...
                logger.info(f"Updated {self.display_name} policy: {name}")
                
            except Exception as e:
                error_msg = f"Failed to update {self.display_name} policy '{name}': {e}"
                logger.error(error_msg)
                results['errors'].append(error_msg)
                results['error_count'] += 1
    ...
    def delete_objects(self, to_delete: set) -> Dict[str, Any]:
        ...
        for name in to_delete:
            try:
                ...
                if safe_mode:
                    warning_msg = f"SAFE MODE: Would delete {self.display_name} policy '{name}' (set SAFE_MODE=false to allow deletions)"
                    logger.warning(warning_msg)
                    results['warnings'].append(warning_msg)
                else:
                    # Check if policy can be safely deleted (no profiles using it)
                    can_delete = self._can_delete_policy(current_obj)
                    logger.debug(f"Policy '{name}' deletion check: can_delete={can_delete}, profiles={current_obj.get('Profiles', [])}")
                    if can_delete:
                        self.api_client.delete_object(self.object_type, current_obj['moid'])
                        results['deleted_count'] += 1
                        logger.info(f"Deleted {self.display_name} policy: {name}")
                    else:
                        warning_msg = f"Cannot delete {self.display_name} policy '{name}': policy is in use by profiles"
                        logger.warning(warning_msg)
                        results['warnings'].append(warning_msg)
                
            except Exception as e:
                error_msg = f"Failed to delete {self.display_name} policy '{name}': {e}"
                logger.error(error_msg)
                results['errors'].append(error_msg)
                results['error_count'] += 1
    ...
    def document(self) -> Dict[str, Any]:
        """
        Generate documentation for the policy object type.
        ...
        try:
            ...
            example = {
                ...
                'Description': f'Example {self.display_name} policy for development environment',
                'Organization': 'default'
            }
            
            # Add policy-specific example fields
            example.update(self._get_example_policy_fields())
            
            return {
                'object_type': self.object_type,
                'display_name': self.display_name,
                'description': f'{self.display_name} policies for Cisco Intersight infrastructure management',
    ...
    def _process_references_for_import(self, obj: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'Organization' in processed:
            ...
            if org_name and self.api_client:
                try:
                    # Use organization resolver for better performance and fallback strategies
                    org_moid = None
    ...
    def validate_organization_reference(self, obj: Dict[str, Any]) -> List[str]:
        """
        Validate organization references in a policy object.
        
        Args:
            obj: Policy object to validate
        ...
        # Handle object organization reference (legacy format)
            ...
            if 'Name' in org_ref:
                ...
                if not org_moid or not org_moid.strip():
                    ...
                    if not self.api_client.organization_resolver.resolve_moid_to_name(org_moid):
                        errors.append(f"Organization MOID '{org_moid}' cannot be resolved to a valid name")
            else:
    ...
    def _objects_are_equivalent(self, current_obj: Dict[str, Any], yaml_obj: Dict[str, Any]) -> bool:
        """
        Check if two policy objects are equivalent.
    ...
    def _can_delete_policy(self, policy_obj: Dict[str, Any]) -> bool:
        """
        Check if a policy can be safely deleted.
        
        Args:
            policy_obj: Policy object dictionary
            
        Returns:
            True if policy can be deleted
        """
        # Check if policy is associated with any profiles
        profiles = policy_obj.get('Profiles', [])
        if profiles:
            logger.debug(f"Policy '{policy_obj.get('Name')}' is used by {len(profiles)} profiles")
            return False
    ...
    def _extract_user_defined_values(self, obj: Dict[str, Any]) -> Dict[str, Any]:
        """
        Extract user-defined values from policy, including filtered user tags.
