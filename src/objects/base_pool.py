"""
Base Pool class for all Intersight pool objects.

This module provides the base Pool class that all specific pool 
implementations inherit from. It handles common pool attributes and
provides a foundation for pool-specific implementations.
"""
...
class BasePool(ObjectType):
    """
    Base class for all Intersight pool objects.
    
    This class provides common functionality for all pool types including:
    - Organization field handling
    - Common pool attributes (Name, Description, Tags)
    - Pool assignment and block management
    - Pool-specific validation and transformation
    """
    
    def __init__(self, api_client=None, openapi_schema=None, validation_level=ValidationLevel.STRICT):
        """Initialize the base pool object."""
    ...
    def folder_path(self) -> str:
        """Return the base folder path for pools."""
        return "pools"
    
    def _get_common_pool_fields(self) -> Dict[str, FieldDefinition]:
        """
        Get common fields that all pools share.
        
        Returns:
            Dictionary of common pool field definitions
        """
        return {
    ...
    def generate_filename(self, obj: Dict[str, Any], organization_name: str = None) -> str:
        """
        Generate a filename for a pool object.
        
        Pools are named with organization prefix: {org_name}_{pool_name}.yaml
        
        Args:
            obj: The pool object dictionary
    ...
    def export(self, output_dir: str) -> Dict[str, Any]:
        """
        Export pools from Intersight to YAML files.
        ...
        try:
            ...
            # Query all pools from Intersight
            raw_objects = self.api_client.query_objects(self.object_type)
            logger.info(f"Retrieved {len(raw_objects)} raw {self.display_name} pools from Intersight")
            
            # Filter out system default pools
            filtered_objects = self.filter_system_defaults(raw_objects)
            logger.info(f"Filtered to {len(filtered_objects)} user-defined {self.display_name} pools")
            
            # Only create directory when we have objects to export
            if not filtered_objects:
                logger.info(f"No user-defined {self.display_name} pools to export, skipping folder creation")
                return results
            
            # Process each pool
            for pool_obj in filtered_objects:
                try:
                    # Extract user-defined values
                    user_data = self._extract_user_defined_values(pool_obj)
                    ...
                    pool_dir = os.path.join(output_dir, self.folder_path)
                    ...
                    logger.debug(f"Exported {self.display_name} pool '{processed_data.get('Name')}' to {file_path}")
                    
                except Exception as e:
                    error_msg = f"Failed to export {self.display_name} pool '{pool_obj.get('Name', 'unknown')}': {e}"
            ...
            error_msg = f"Failed to export {self.display_name} pools: {e}"
            logger.error(error_msg)
            results['errors'].append(error_msg)
            results['error_count'] += 1
    ...
    def import_objects(self, yaml_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Import pools from YAML data to Intersight.
        
        Args:
            yaml_data: List of pool dictionaries from YAML files
        ...
        try:
            ...
            # Get current pools from Intersight
            ...
            # Create lookup maps (API might use lowercase 'name' like organizations, policies, and profiles)
            ...
            logger.debug(f"Current {self.display_name} pools from API: {list(current_by_name.keys())}")
            logger.debug(f"YAML {self.display_name} pools: {list(yaml_by_name.keys())}")
            ...
            for name in to_create:
                try:
                    ...
                    if validation_errors:
                        error_msg = f"Validation failed for {self.display_name} pool '{name}': {validation_errors}"
                        logger.error(error_msg)
                        results['errors'].append(error_msg)
                        results['error_count'] += 1
                        continue
                    ...
                    logger.info(f"Created {self.display_name} pool: {name}")
                    
                except Exception as e:
                    error_msg = f"Failed to create {self.display_name} pool '{name}': {e}"
                    logger.error(error_msg)
                    results['errors'].append(error_msg)
                    results['error_count'] += 1
            ...
            for name in to_update:
                try:
                    ...
                    if self._objects_are_equivalent(current_obj, yaml_obj):
                        logger.debug(f"{self.display_name} pool '{name}' is already up to date")
                        continue
                    ...
                    # Add organization-specific validation
                    org_validation_errors = self.validate_organization_reference(yaml_obj)
                    validation_errors.extend(org_validation_errors)
                    ...
                    logger.info(f"Updated {self.display_name} pool: {name}")
                    
                except Exception as e:
                    error_msg = f"Failed to update {self.display_name} pool '{name}': {e}"
                    logger.error(error_msg)
                    results['errors'].append(error_msg)
                    results['error_count'] += 1
            
            # Handle deletions (pools can be sensitive to delete if they have allocations)
            ...
            for name in to_delete:
                try:
                    ...
                    if safe_mode:
                        warning_msg = f"SAFE MODE: Would delete {self.display_name} pool '{name}' (set SAFE_MODE=false to allow deletions)"
                        logger.warning(warning_msg)
                        results['warnings'].append(warning_msg)
                    else:
                        # Check if pool can be safely deleted
                        if self._can_delete_pool(current_obj):
                            self.api_client.delete_object(self.object_type, current_obj['moid'])
                            results['deleted_count'] += 1
                            logger.info(f"Deleted {self.display_name} pool: {name}")
                        else:
                            warning_msg = f"Cannot delete {self.display_name} pool '{name}': pool has active allocations"
                            logger.warning(warning_msg)
                            results['warnings'].append(warning_msg)
                    
                except Exception as e:
                    error_msg = f"Failed to delete {self.display_name} pool '{name}': {e}"
                    logger.error(error_msg)
                    results['errors'].append(error_msg)
                    results['error_count'] += 1
            
            logger.info(f"{self.display_name} import completed: {results['created_count']} created, "
                       f"{results['updated_count']} updated, {results['deleted_count']} deleted, "
                       f"{results['error_count']} errors")
            
        except Exception as e:
            error_msg = f"Failed to import {self.display_name} pools: {e}"
            logger.error(error_msg)
            results['errors'].append(error_msg)
            results['error_count'] += 1
    ...
    def document(self) -> Dict[str, Any]:
        """
        Generate documentation for the pool object type.
        ...
        try:
            ...
            example = {
                ...
                'Description': f'Example {self.display_name} pool for development environment',
                'Organization': 'default'
            }
            
            # Add pool-specific example fields
            example.update(self._get_example_pool_fields())
            
            return {
                'object_type': self.object_type,
                'display_name': self.display_name,
                'description': f'{self.display_name} pools for Cisco Intersight infrastructure management',
    ...
    def validate_organization_reference(self, obj: Dict[str, Any]) -> List[str]:
        """
        Validate organization references in a pool object.
        
        Args:
            obj: Pool object to validate
    ...
    def _objects_are_equivalent(self, current_obj: Dict[str, Any], yaml_obj: Dict[str, Any]) -> bool:
        """
        Check if two pool objects are equivalent.
    ...
    def _can_delete_pool(self, pool_obj: Dict[str, Any]) -> bool:
        """
        Check if a pool can be safely deleted.
        
        Args:
            pool_obj: Pool object dictionary
            
        Returns:
            True if pool can be deleted
        """
        # Check if pool has any allocated members
        # This is a basic implementation - specific pool types may have additional checks
        
        # Check allocation status
        allocated = pool_obj.get('Allocated', 0)
        total_size = pool_obj.get('Size', 0)
        
        if allocated > 0:
            logger.debug(f"Pool '{pool_obj.get('Name')}' has {allocated} allocated members out of {total_size}")
            return False
    ...
    def _extract_user_defined_values(self, obj: Dict[str, Any]) -> Dict[str, Any]:
        """
        Extract user-defined values from pool, including filtered user tags.
